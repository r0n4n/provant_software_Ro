...
public:
	...
	int get_raw_size();
	void serializeFloat(float a);

	void multwii_attitude(float x,float y,float z);
	void multwii_raw_imu(float* acc,float* gyr, float* mag);
	void multwii_altitude(float alt, float vario);
	void multwii_bicopter_identifier();
	void multwii_motor_pins();
	void multwii_motor(float forca_esquerdo,float forca_direito);
	void multwii_servos(float angle1,float angle2);
...
//	void multwii2_sendControldatain(float rpy[3], float drpy[3], float position[3], float velocity[3]);
	void multwii2_sendControldatain(float rpy[3], float drpy[3], float position[3], float velocity[3], float alpha[2], float dotAlpha[2]);
